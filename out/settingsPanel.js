...
class SettingsPanel {
    ...
    async detectAndSendCompilers() {
        // Импортируем CompilerManager
        const { CompilerManager } = await Promise.resolve().then(() => __importStar(require('./compilerManager')));
    ...
    async sendCurrentSettings() {
        const config = vscode.workspace.getConfiguration('cpp-asm-viewer');
        const settings = {
            compilerArgs: config.get('compilerArgs') || '/Od /FAcs /c /EHsc',
            selectedCompiler: config.get('selectedCompiler') || '',
