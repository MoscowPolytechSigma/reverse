...
export class CompilerManager2 {
    ...
    private async compileWithMSVC(compiler: CompilerInfo, sourcePath: string): Promise<CompilationResult> {
    ...
    `;
        ...
        private escapePath(pathStr: string): string {
    ...
    private checkCompilationResult(
    ...
    private cleanAssemblyOutput(assembly: string): string {
        const lines = assembly.split('\n');
    ...
    private async compileWithGCCLike(compiler: CompilerInfo, sourcePath: string): Promise<CompilationResult> {
    ...
    private parseMappings(assembly: string, sourcePath: string): LineMapping[] {
        ...
        const mappingDict: Map<number, number[]> = new Map();
    ...
    private parseMappingsAlternative(assembly: string, sourcePath: string): LineMapping[] {
        const mappings: LineMapping[] = [];
        const lines = assembly.split('\n');
        const colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FFEAA7', '#DDA0DD', '#98D8C8', '#F7DC6F'];
        ...
        let currentAssemblyLines: number[] = [];
    ...
    private isRealAssemblyInstruction(line: string): boolean {
    ...
    private isAssemblyInstruction(line: string): boolean {
        const trimmed = line.trim();
    ...
    private mergeScatteredMappings(mappings: LineMapping[]): LineMapping[] {
    ...
    private filterInvalidMappings(mappings: LineMapping[], assemblyLines: string[]): LineMapping[] {
        const filtered: LineMapping[] = [];
