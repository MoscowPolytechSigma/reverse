...
export class CompilerManager {
    ...
    private parseMappings(assembly: string, sourcePath: string): LineMapping[] {
        ...
        let currentFileInComments: string | null = null;
    ...
    private isAssemblyInstruction(line: string): boolean {
        ...
        const patterns = [
            /^[0-9A-F]+\s+[0-9A-F]+\s+/, // Адрес + машинный код + инструкция
            /^\t[a-z]+\s+/i,             // Табуляция + инструкция + операнды
            /^[a-z]+\s+[a-z0-9]+,/i,     // Инструкция + операнд,
    ...
    private hasLineDirective(line: string): boolean {
        const trimmed = line.trim();
        return /;\s*Line\s+\d+/i.test(trimmed) || /;\s*\d+\s*:/.test(trimmed) || /;\s*\d+\s+\|/.test(trimmed);
    }

    private createFallbackMappings(assemblyLineCount: number): LineMapping[] {
        const mappings: LineMapping[] = [];
